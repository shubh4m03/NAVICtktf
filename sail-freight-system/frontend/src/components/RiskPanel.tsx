...
function riskColorClass(value: number): string {
  if (value <= 0.33) return 'bg-status-green'
  if (value <= 0.66) return 'bg-status-amber'
    ...
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className={`text-2xs font-medium ${value <= 0.33 ? 'text-status-green' : value <= 0.66 ? 'text-status-amber' : 'text-status-red'}`}>
            {riskLabel(value)}
          </span>
        </div>
      </div>
  ...
  return (
    <div className="card panel-enter flex flex-col" role="region" aria-label="Risk Assessment">
      ...
      <div className="p-3 flex flex-col gap-3 flex-1">
        ...
        {/* Overall summary */}
        <div className="rounded bg-surface border border-border p-2 flex items-center justify-between">
