...
export const MaritimeGlobe: React.FC<MaritimeGlobeProps> = ({
  ...
  useEffect(() => {
    ...
    map.on('click', 'chokepoints-layer', (e: maplibregl.MapLayerMouseEvent) => {
      ...
      const foundCp = MARITIME_CHOKEPOINTS.find((c) => c.id === chokepointId);
  ...
  const updateChokepointsLayer = (map: maplibregl.Map) => {
    const source = map.getSource('chokepoints-source') as maplibregl.GeoJSONSource | undefined;
    if (!source) return;

    const features = MARITIME_CHOKEPOINTS.map((cp) => ({
      type: 'Feature' as const,
      properties: {
        id: cp.id,
        name: cp.name,
  ...
  return (
    <div className="relative w-full h-full min-h-[460px] overflow-hidden rounded bg-background-secondary border border-border select-none">
      ...
      <div className="absolute top-3.5 left-3.5 z-10 pointer-events-none bg-surface/90 backdrop-blur-sm border border-border px-3.5 py-2.5 rounded shadow-subtle max-w-xs font-mono">
        ...
        <div className="flex items-center gap-3 text-[11px]">
          <div>
            ...
            <span className="text-primary font-semibold truncate block max-w-[170px]">
      ...
      <div className="absolute top-3.5 right-3.5 z-10 flex flex-col gap-1.5 font-mono text-[11px]">
        <button
          ...
          className={`px-2.5 py-1.5 rounded border flex items-center justify-between gap-2 transition-colors shadow-subtle ${
            isGlobeProjection
              ...
              : 'bg-surface/90 border-border text-ink-secondary hover:text-ink'
          }`}
        >
        ...
        <button
          ...
          className="px-2.5 py-1.5 rounded border bg-surface/90 border-border text-ink-secondary hover:text-ink hover:bg-surface-elevated transition-colors shadow-subtle flex items-center justify-between gap-2"
        >
      ...
      <div className="absolute bottom-3 left-3 right-3 z-10 pointer-events-none flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
        {/* Maritime Legend */}
        <div className="pointer-events-auto bg-surface/90 backdrop-blur-sm border border-border px-3 py-1.5 rounded shadow-subtle flex items-center gap-3 text-[10px] font-mono">
          ...
          <div className="flex items-center gap-1.5">
            <span className="w-5 h-1 bg-primary inline-block rounded" />
            <span className="text-ink font-semibold">Geodesic Route</span>
          </div>
        </div>

        {/* Demo Data Disclaimer Badge */}
        <div className="pointer-events-auto bg-surface/95 backdrop-blur-sm border border-border px-3 py-1.5 rounded shadow-subtle flex items-center gap-1.5 text-[10px] font-mono text-ink-muted">
