...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4 bg-black/50 backdrop-blur-xs animate-fadeIn">
      <div
        className="w-full max-w-xl rounded-md bg-surface border border-border shadow-2xl overflow-hidden flex flex-col font-mono"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-background-secondary/50">
          ...
          />
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-surface border border-border text-ink-muted">
            ESC
          </span>
          <button
            onClick={onClose}
          ...
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map((item) => (
              <button
                key={item.id}
                onClick={item.action}
                className="w-full px-3 py-2 rounded text-left flex items-center justify-between gap-3 hover:bg-background-secondary transition-colors group"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-6 h-6 rounded bg-surface border border-border flex items-center justify-center shrink-0 text-primary">
                ...
                <ArrowRight className="w-3.5 h-3.5 text-ink-muted group-hover:text-primary group-hover:translate-x-0.5 transition-all shrink-0" />
              </button>
            ))
          )}
        </div>
        ...
        {/* Footer */}
        <div className="px-4 py-2 border-t border-border bg-background-secondary/30 flex items-center justify-between text-[10px] text-ink-muted">
          <span>NAVIC Maritime Directory</span>
          <div className="flex items-center gap-2">
            <span>↑↓ Navigate</span>
            <span>↵ Select</span>
          </div>
