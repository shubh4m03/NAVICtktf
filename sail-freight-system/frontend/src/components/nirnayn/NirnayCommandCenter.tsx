...
export default function NirnayCommandCenter() {
  ...
  const [cargoWeight, setCargoWeight] = useState(70000);
  const [selectedRoute, setSelectedRoute] = useState(() => {
    const ctx = nirnaynService.getActiveContext();
    return ctx.scenario ? `${ctx.scenario.origin} → ${ctx.scenario.destination}` : 'Hay Point → Paradip';
  });
  const [selectedVessel, setSelectedVessel] = useState(() => {
    return nirnaynService.getActiveContext().vesselClass || 'Panamax';
  });

  React.useEffect(() => {
    const unsubscribe = nirnaynService.subscribe((ctx) => {
      if (ctx.scenario) {
        setSelectedRoute(`${ctx.scenario.origin} → ${ctx.scenario.destination}`);
        if (ctx.scenario.commodity) setCargoType(ctx.scenario.commodity);
        if (ctx.scenario.cargoTonnage) setCargoWeight(ctx.scenario.cargoTonnage);
      }
      if (ctx.vesselClass) setSelectedVessel(ctx.vesselClass);
    });
    return unsubscribe;
  }, []);
  ...
  return (
    <div className="flex-1 flex overflow-hidden bg-background">
      ...
      <aside className="w-64 hidden md:flex flex-col border-r border-border bg-surface flex-shrink-0">
        <div className="px-3.5 py-2.5 border-b border-border bg-background-secondary flex items-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto p-2.5 space-y-1.5">
          {mockRecentInquiries.map((inq) => (
            <div
              ...
              className="p-2 rounded border border-border hover:border-border-hover bg-background-secondary hover:bg-surface-elevated cursor-pointer transition-colors text-xs"
            >
              ...
              <div className="flex justify-between items-center text-[10px] font-mono text-ink-muted mt-1">
                <span>{inq.date}</span>
                <span className="text-primary">{inq.status}</span>
              </div>
            </div>
          ))}

          <div className="pt-3 border-t border-border mt-3">
            <span className="text-[10px] font-mono uppercase tracking-wider text-ink-secondary block px-1 mb-1.5">
              Decision Presets
            </span>
            <div className="space-y-1">
              {promptPresets.map((prompt, idx) => (
                <button
                  ...
                  className="w-full text-left p-1.5 rounded hover:bg-background-secondary text-[11px] text-ink-secondary hover:text-ink transition-colors leading-snug line-clamp-2"
                >
                  › {prompt}
                </button>
              ))}
      ...
      <div className="flex-1 flex flex-col p-4 lg:p-6 overflow-y-auto">
        <header className="mb-4 border-b border-border pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            {/* Minimal Navigation Decision Node Mark */}
            <div className="w-7 h-7 rounded bg-primary/15 border border-primary/30 flex items-center justify-center text-primary">
          ...
          <div className="text-xs font-mono text-ink-muted">
            ENGINE: <span className="text-primary font-semibold">NIRNAY CORE v2.4</span>
          </div>
        </header>
        ...
        <div className="flex-1 flex flex-col justify-between space-y-4">
          ...
          {!response && !isAnalyzing && (
            <div className="card p-6 max-w-xl mx-auto my-auto space-y-3.5 border border-border shadow-panel">
              ...
              <div className="grid grid-cols-2 gap-2 pt-1 text-xs font-mono">
                <div className="p-2.5 rounded bg-background-secondary border border-border">
                  <span className="text-[10px] text-ink-muted block uppercase font-bold">PHYSICAL CHECKS</span>
                  <span className="text-ink">Draft, DWT, Beam, Canal Locks</span>
                </div>
                <div className="p-2.5 rounded bg-background-secondary border border-border">
                  <span className="text-[10px] text-ink-muted block uppercase font-bold">COMMERCIAL CHECKS</span>
          ...
          {response && (
            <div className="card p-5 space-y-4 shadow-panel border border-border">
              {/* Header */}
              <div className="flex items-center justify-between border-b border-border pb-3">
                ...
                <div className="flex items-center gap-2 text-xs font-mono">
                  ...
                  <span className="text-status-green font-semibold bg-status-green-bg px-2 py-0.5 rounded border border-status-green-border">
              ...
              <div className="p-3.5 rounded bg-background-secondary border border-border space-y-1">
              ...
              <div className="space-y-2">
                <span className="text-[10px] text-ink-secondary font-mono uppercase tracking-wider block">
                ...
                <div className="space-y-1.5">
                  {response.rationale.map((r, i) => (
                    <div
                      key={i}
                      className="text-xs text-ink flex items-start gap-2.5 p-2 rounded bg-background-secondary border border-border"
                    >
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {response.tradeoffs.map((t, i) => (
                      <div
                        key={i}
                        className="text-xs text-ink-secondary flex items-start gap-2 p-2 rounded bg-background-secondary/60 border border-border/60"
                      >
              ...
              {/* Action Bar */}
              <div className="flex justify-between items-center pt-3 border-t border-border text-xs font-mono">
          ...
          {/* Loading State */}
          {isAnalyzing && (
            <div className="card p-8 flex flex-col items-center justify-center space-y-2.5 border border-border">
        ...
        <div className="mt-4 relative">
          ...
          />
          <button
            className="absolute right-1.5 top-1.5 px-3 py-1 bg-primary hover:bg-maritime text-white rounded text-xs font-mono flex items-center gap-1.5 transition-colors disabled:opacity-50"
            onClick={() => handleAsk()}
      ...
      <aside className="w-72 hidden xl:flex flex-col p-3.5 bg-surface border-l border-border space-y-3 flex-shrink-0 overflow-y-auto">
        <div className="flex items-center gap-1.5 text-xs font-mono font-bold text-ink uppercase tracking-wider pb-2 border-b border-border">
          <Activity className="w-3.5 h-3.5 text-primary" /> Active Telemetry Context
        </div>

        {/* Cargo Specification */}
        <div className="card p-3 space-y-1.5">
          <span className="text-[10px] font-mono uppercase text-ink-secondary">Cargo Profile</span>
          <div className="space-y-1 text-xs font-mono">
            <div className="flex justify-between">
              <span className="text-ink-secondary">Commodity:</span>
              <span className="text-ink font-semibold">{cargoType}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-ink-secondary">Volume:</span>
              <span className="text-primary font-semibold">{cargoWeight.toLocaleString()} MT</span>
            </div>
          </div>
        </div>
        ...
        {/* Voyage Corridor */}
        <div className="card p-3 space-y-1.5">
          <span className="text-[10px] font-mono uppercase text-ink-secondary">Trade Lane</span>
          <div className="space-y-1 text-xs font-mono">
            <div className="flex justify-between">
              <span className="text-ink-secondary">Origin:</span>
              <span className="text-ink">Hay Point (AUS)</span>
            </div>
            <div className="flex justify-between">
              <span className="text-ink-secondary">Destination:</span>
              <span className="text-ink">Paradip (IND)</span>
            </div>
            <div className="flex justify-between">
              <span className="text-ink-secondary">Max Berth Draft:</span>
              <span className="text-status-warning font-semibold">14.5 m</span>
            </div>
          </div>
        </div>
        ...
        {/* Carrier Designation */}
        <div className="card p-3 space-y-1.5">
          <span className="text-[10px] font-mono uppercase text-ink-secondary">Designated Carrier</span>
          <div className="space-y-1 text-xs font-mono">
            <div className="flex justify-between">
              <span className="text-ink-secondary">Class:</span>
              <span className="text-ink font-semibold">{selectedVessel}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-ink-secondary">Operating Draft:</span>
              <span className="text-primary font-semibold">14.2 m (Compliant)</span>
            </div>
          </div>
        </div>
        ...
        {/* Market Momentum */}
        <div className="card p-3 space-y-1.5">
          <span className="text-[10px] font-mono uppercase text-ink-secondary">Market Momentum</span>
          <div className="flex items-center justify-between text-xs font-mono">
            <span className="text-ink-secondary">Freight Trend:</span>
            <span className="text-status-green font-semibold">+12.4% (RISING)</span>
          </div>
        </div>
      </aside>
    </div>
